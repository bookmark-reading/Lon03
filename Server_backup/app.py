...
class TranscriptHandler(TranscriptResultStreamHandler):
    ...
    def __init__(self, transcript_result_stream, client_id, server_instance):
        ...
        self.processed_transcripts = set()  # Track processed transcripts to avoid duplicates
        self.last_cleanup_time = datetime.now()
        self.cleanup_interval_seconds = 60  # Clear old entries every 60 seconds
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        # Periodic cleanup of processed transcripts to prevent memory buildup
        if (datetime.now() - self.last_cleanup_time).total_seconds() > self.cleanup_interval_seconds:
            self.processed_transcripts.clear()
            self.last_cleanup_time = datetime.now()
            print(f"[CLEANUP] Cleared processed transcripts cache")
        
        for result in results:
            # Only process final results
            if not result.is_partial:
                # Get result ID to track duplicates
                ...
                for alt in result.alternatives:
                    ...
                    # Deduplicate by result_id if available
                    if result_id:
                        if result_id in self.processed_transcripts:
                            print(f"[SKIPPED DUPLICATE] Result ID: {result_id}")
                            continue
                        self.processed_transcripts.add(result_id)
                    else:
                        # Fallback: deduplicate by transcript text + timestamp
                        dedup_key = f"{transcript}_{datetime.now().timestamp()}"
                        if transcript in self.processed_transcripts:
                            print(f"[SKIPPED DUPLICATE] '{transcript[:50]}...'")
                            continue
                        self.processed_transcripts.add(transcript)
